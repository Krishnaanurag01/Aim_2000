...
public class Q11_K_partitions {
	...
	public static void solution(int i, int n, int k, int ssf, ArrayList<ArrayList<Integer>> ans) {
		...
		if(ssf == k){ // if set so far(ssf) equals to k means if we got all the k subset then print the answer
			counter++ ;
		...
		for(int j = 0 ; j < ans.size() ; j++){
			...
			else{ // else if set doesn't have any element in it then add current element in the first empty set (as we don't want permutation)
				ans.get(j).add(i) ;
				solution(i+1,n,k,ssf+1,ans) ; // as we added one element in the empty set so set so far will get increased by 1.
				ans.get(j).remove(ans.get(j).size() - 1) ; 
				break ; // this will help us in avoiding the permuation and adding element in only one set
			}
		}
	}
